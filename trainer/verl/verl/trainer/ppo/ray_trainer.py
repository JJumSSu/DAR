    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "reward_extra_info" in result:
                for k, v in result["reward_extra_info"].items():
                    metrics[k] = v
                result["reward_extra_info"].clear()
        ...
        if len(metrics) > 0:
            for k, v in metrics.items():
                if 'rmse' in k:
                    metric_dict[f"val-core/reward/{k}"] = np.float64(v).item()
                metric_dict[f"val-aux/reward/{k}"] = np.float64(v).item()
        else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if isinstance(reward_extra_infos_dict, dict) and reward_extra_infos_dict:
                        for k, v in reward_extra_infos_dict.items():
                            metrics.update({f"training/analysis-of-rollouts-{k}": float(v)})
                        reward_extra_infos_dict.clear()
