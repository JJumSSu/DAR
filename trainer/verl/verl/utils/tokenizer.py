...
def hf_tokenizer(name_or_path, correct_pad_token=True, correct_gemma2=True, **kwargs):
    ...
    maybe_gemma2 = correct_gemma2 and isinstance(name_or_path, str) and "gemma-2-2b-it" in name_or_path.lower()
    if maybe_gemma2:
    ...
    # Local checkpoints may not contain "gemma-2-2b-it" in the path; detect by tokenizer class as fallback.
    if correct_gemma2 and not maybe_gemma2 and "gemma" in tokenizer.__class__.__name__.lower():
        end_of_turn_id = tokenizer.convert_tokens_to_ids("<end_of_turn>")
        if isinstance(end_of_turn_id, int) and end_of_turn_id >= 0:
            tokenizer.eos_token = "<end_of_turn>"
            tokenizer.eos_token_id = end_of_turn_id
            warnings.warn(
                f"Detected Gemma tokenizer ({tokenizer.__class__.__name__}). "
                f"Set eos_token to <end_of_turn> and eos_token_id to {end_of_turn_id}.",
                stacklevel=1,
            )
